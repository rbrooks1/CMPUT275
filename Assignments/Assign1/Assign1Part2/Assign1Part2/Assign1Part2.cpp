...
// initialization of the two structs
...
// function that sets all the indices to -1 and all
// the distances to 30000 as a check to end the printing
// of mode 1
void restClear() {
...
// prints the values for the sort selection button
...
void setup() {
  ...
  // print the text for the rating and sort select
  // buttons
  sortPrint(sortVal % 3);
  tft.setTextSize(2);
...
void processTScreen() {
  ...
  // increments and reprints the rating value to the lcd screen
  if ((screen_x > 274) && (screen_x < 320) && (screen_y > 0) && (screen_y < 120)) {
    ...
    tft.print((rateVal % 5) + 1);
    delay(500);
    // changes and reprints the sorting method
...
// the next three functions are for quicksort
...
// prints the restaurants 30 at a time
// if the user goes below the 30th restaurant then
// the next 30 restaurants are taken and printed.
// no wrap around.
...
// only reads in the restaurants that are above or equal to
// the rating value
void ratingControl(int i) {
    if (floor((rest.rating + 1)/2.0) >= (rateVal % 5) + 1) {
          rest_dist[sortIndex].index = i;
          int cursorDispX = yegCurrentX + cursorX;
          int cursorDispY = yegCurrentY + cursorY;
          int restx = lon_to_x(rest.lon);
          int resty = lat_to_y(rest.lat);
          rest_dist[sortIndex].dist = abs(restx - cursorDispX) + abs(resty - cursorDispY);
          sortIndex++;
        }
}
...
void processRest() {
  ...
  for (int i = 0; i < NUM_RESTAURANTS; i++) {
    ...
    Serial.print(sortIndex);
  // the array of RestDists is sorted based on manhattan distances.
  // and the times it takes for the sort to happen is printed to the
  // serial monitor
  if ((sortVal % 3) == 0) {
    uint32_t initTime = millis();
    quickSort(rest_dist, 0, sortIndex);
  ...
  int t = 0;
    // prints the closest 30 restaurants and if the user goes beyond
    // 30 the next closest 30 restaurants are printed
    ...
    while (true) {
      ...
      if (joySelect == LOW) {
        flag = 0;
          ...
          Serial.println(x);
          Serial.println(y);
          Serial.println(cursorX + yegCurrentX);
          Serial.println(cursorY + yegCurrentY);
          ...
          if ((y - MAP_DISP_HEIGHT/2) > 0 && (y - MAP_DISP_HEIGHT/2) < (LCD_HEIGHT - MAP_DISP_HEIGHT)) {
            ...
            // ensures the screen stays in the map boundaries.
              ...
              if (y < LCD_HEIGHT) {
                ...
                cursorY = LCD_HEIGHT - CURSOR_SIZE;
              }
          }
          tft.setTextColor(0xFFFF , 0x0000);
          tft.fillScreen(ILI9341_BLACK);
